...
const ampm = (h, m) => { const ap = h < 12 ? "AM" : "PM"; const h12 = h % 12 || 12; return `${h12}:${String(m).padStart(2, "0")} ${ap}`; };
...
writeMem("user/alex-profile.md", {
  name: "Alex Rivera", description: "Who Alex is — indie developer, builder of small apps",
  type: "user", importance: 9, tags: ["identity", "profile"], origin: "chat",
...
writeMem("user/preferences.md", {
  name: "Preferences", description: "Tooling and workflow preferences",
  type: "user", importance: 7, tags: ["preferences", "tooling"],
...
writeMem("project/nimbus-notes.md", {
  name: "Nimbus Notes", description: "Offline-first note-taking app",
  type: "project", importance: 8, tags: ["app", "react-native", "active"],
...
writeMem("project/atlas-api.md", {
  name: "Atlas API", description: "Sync + auth backend for Nimbus Notes",
  type: "project", importance: 7, tags: ["api", "backend", "active"],
...
writeMem("project/wandr-travel.md", {
  name: "Wandr (travel)", description: "Paused side project — trip planner",
  type: "project", importance: 4, tags: ["app", "paused"],
}, `
# Wandr
...
writeMem("decisions/postgres-over-mongo.md", {
  name: "Postgres over Mongo", description: "Chose Postgres for Atlas API",
  type: "decision", importance: 6, tags: ["architecture", "database"],
  related: ["project/atlas-api.md"], valid_from: ymd(daysAgo(35)),
}, `
...
writeMem("decisions/monorepo.md", {
  name: "Monorepo decision", description: "Single repo for app + API + shared types",
  type: "decision", importance: 6, tags: ["architecture", "tooling"],
...
writeMem("feedback/terse-responses.md", {
  name: "Prefers terse responses", description: "How assistants should communicate with Alex",
  type: "feedback", importance: 7, tags: ["communication", "style"],
}, `
## Communication style
...
writeMem("feedback/test-first.md", {
  name: "Test-first for risky code", description: "Write tests before touching sync/auth",
  type: "feedback", importance: 6, tags: ["process", "testing"],
}, `
...
writeMem("references/design-tokens.md", {
  name: "Design tokens", description: "Link to the shared design tokens",
  type: "reference", importance: 5, tags: ["design", "reference"],
}, `
## Design tokens
...
writeMem("references/oss-licenses.md", {
  name: "OSS license cheatsheet", description: "Quick license compatibility notes",
  type: "reference", importance: 4, tags: ["legal", "reference"],
}, `
...
writeMem("reflection/q2-themes.md", {
  name: "Q2 themes", description: "Synthesized themes from recent work",
  type: "reflection", importance: 8, tags: ["reflection", "planning"],
...
writeMem("sessions/2026-05-28-review.md", {
  name: "Session review — 2026-05-28", description: "5th-session self-assessment",
  type: "session", importance: 6, tags: ["review", "meta"],
...
function dailyLog(dayOffset) {
  ...
  for (let i = 0; i < 1 + Math.floor(rng() * 2); i++) {
    ...
    ev(10 + i, 15, "Email", subj, `From: ${pick(contacts)}@example.com`, subj.includes("Invoice") ? 6 : 4);
  }
  // imessage
  if (chance(0.7)) {
    ev(12, 30, "iMessage", `${pick(contacts)}: lunch then pair on the merge bug?`, "Chat: direct", 5);
  }
  // a redacted local-only example (shows privacy enforcement)
  if (dayOffset === 3) {
    ev(13, 5, "Email", "[redacted — local-only: content classified as sensitive and withheld from the synced/embedded store]", "From: bank@example.com", 5, "local-only");
  }
  // orchestrator agent results
  for (let i = 0; i < 1 + Math.floor(rng() * 2); i++) {
    const [agent, msg] = pick(agents);
    const failed = chance(0.15);
    ev(14 + i, 40, "orchestrator", `${agent}: ${failed ? "FAILED — " + msg : msg}`, `> meta: agent_id=${agent}`, failed ? 8 : 6);
  }
  // fused activity
  if (chance(0.4)) {
    ev(15, 20, "Activity (fused)", "Cross-source: design review meeting + 3 Figma comments + a follow-up email within 30 min.", "sources: calendar, email", 6);
  }
  // a manual journal entry on some days
  if (chance(0.5)) {
    const mood = pick(["great", "good", "neutral", "tired"]);
    ev(18, 0, `Journal *(${mood})*`, pick([
  ...
  if (dayOffset === 1) {
    lines.push(`> **Note — ${date} (via dashboard):** Reminder: demo this to the team Friday. Links back to [[Q2 themes]].`, "");
  }

  fs.mkdirSync(path.join(MEM, "daily"), { recursive: true });
  fs.writeFileSync(path.join(MEM, "daily", `${date}.md`), header + lines.join("\n") + "\n", "utf-8");
}

for (let i = 0; i < 16; i++) dailyLog(i);

// ── summary ──────────────────────────────────────────────────
const count = (dir) => fs.existsSync(dir) ? fs.readdirSync(dir, { recursive: true }).filter((f) => String(f).endsWith(".md")).length : 0;
...
console.log(`  MEMORIA_API_KEY=demo-key-123 MEMORIA_DIR="${OUT}" MEMORIA_EMBEDDINGS=hash PORT=3110 node dist/http.js`);
